...
class ControllerMatch:
	...
	def print_match(first_player, second_player):
	...
	def print_winner_and_score(first_player, second_player):
	    ...
	    if result in "wW":
	        ...
	        return ViewMatch.print_first_player_wins_and_score(f_p, s_p)

	    elif result in "lL":
	        result = ModelMatch.second_player_wins(match)
	        second_player.score += 1
	        return ViewMatch.print_first_player_wins_and_score(s_p, f_p)

	    elif result in "dD":
	        result = ModelMatch.draw(match)
	        first_player.score += 0.5
	        second_player.score += 0.5
	        return ViewMatch.print_draw(f_p, s_p)    
...
if __name__ == "__main__":
	...
	ControllerMatch.print_match(first_player, second_player)
